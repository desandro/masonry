	...
	$.fn.masonry = function(options, callback) { 
		...
		function masonrySetup(wall, opts, props) {
			if ( props.masoned && opts.newContent != undefined ) {
				// if we're just dealing with newContent
		...
		function masonryArrange(wall, opts, props) {
			...
			if ( props.masoned && opts.newContent != undefined ) {
				// if newContent is set, use colY from last call
		...
		return this.each(function() {  
			...
			if ( !resizeOn && opts.resizeable ) {
				...
				debug(wall.attr('id'), 'resize BINDED');
			}
			if ( resizeOn && !opts.resizeable ) {$(window).unbind('resize.masonry'); debug(wall.attr('id'), 'resize UNbinded');}
